...
class DecoderLayer(nn.Module):
    ...
    """
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
                 bias_mask, layer_dropout=0.0, attention_dropout=0.0, relu_dropout=0.0, multi_input=False,
                 context_size=2, attention_fusion_type='mean'):
        """
        Parameters:
            hidden_size: Hidden size
            total_key_depth: Size of last dimension of keys. Must be divisible by num_head
        ...
        if multi_input and context_size > 1:
            self.context_attentions = nn.ModuleList(MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                       hidden_size, num_heads, None, attention_dropout) for _ in range(context_size))
        else:
        ...
        if multi_input:
            input_sources = context_size + 1
            if self.attention_fusion_type == 'sw':
                self.attention_module = torch.nn.Parameter(torch.ones(input_sources, 1) / input_sources)
            elif self.attention_fusion_type == 'dw':
                self.attention_module = torch.nn.Parameter(torch.ones(input_sources, hidden_size) / input_sources)
            elif self.attention_fusion_type == 'linear':
                self.attention_module = nn.Linear(hidden_size * input_sources, hidden_size)
            elif self.attention_fusion_type == 'dys':
                self.attention_module = nn.ModuleList([nn.Linear(hidden_size, 1) for _ in range(input_sources)])
            elif self.attention_fusion_type == 'dyd':
                self.attention_module = nn.ModuleList([nn.Linear(hidden_size, hidden_size) for _ in range(input_sources)])
            elif self.attention_fusion_type == 'mdys':
                self.attention_module = nn.Linear(hidden_size * input_sources, input_sources)
            elif self.attention_fusion_type == 'mdyd':
                self.attention_module = nn.Linear(hidden_size * input_sources, hidden_size * input_sources)
            elif self.attention_fusion_type == 'att':
                self.attention_module = MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                           hidden_size, num_heads, None, attention_dropout)
                self.attention_module.c_proj.weight = nn.Parameter(torch.ones(hidden_size, hidden_size) / hidden_size)
                self.attention_module.c_proj.bias = nn.Parameter(torch.zeros(hidden_size))
        # self.layer_norm_end = LayerNorm(hidden_size)

    def attention_pooling(self, attention_list, mask):
        if self.attention_fusion_type == "mean":
            ...
            return self.attention_module(attention_list[1], attention_list[2], attention_list[0], mask)
    ...
    def forward(self, inputs):
        ...
        if self.multi_input:
            ...
            for i in range(self.context_size):
                cur_out, cur_attention_weight = self.context_attentions[i](x_norm, encoder_outputs[i],
                                                                                  encoder_outputs[i], mask_src[i])
                ys.append(cur_out)
                attention_weight.append(cur_attention_weight)
            y = self.attention_pooling([y] + ys, mask_src[0])
        else:
        ...
        #         ys.append(cur_out)
        #         attention_weight.append(cur_attention_weight)
        #     y = self.attention_pooling([x_norm] + ys, mask_src[0])
        # else:
        #     # Multi-head encoder-decoder attention
        #     y, attention_weight = self.multi_head_attention_enc_dec(x_norm, encoder_outputs, encoder_outputs, mask_src)

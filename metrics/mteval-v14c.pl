...
#
# version 14c
#    (2016-11-02 lukas.diduch@nist.gov)
#    * Minor bugfixes in scope fixing runtime errors
#
# version 14b
#    (2016-10-25 lukas.diduch@nist.gov)
#    * Fixed bug where leading spaces are begin treated as a token (through using trim function)
#
# version 14a
#    (2016-10-13 lukas.diduch@nist.gov)
#    * Some warning will fail the execution (ensure missing segment labels (ref
#      and hyp) are failing the scoring). Src label names are not being checked.
#    * Avoid tokenizing URL designators in sentence (NOT IMPLEMENTED for intl. tokenization option)
#
# version 14
...
sub score_document
{
    ...
    # score each segment
    # Q: Why do the keys of this hash need to be sorted ?
...
# Wrapper to avoid tokenization of any token starting with a web url designator ("http(s)://")
sub tokenization
{
	my ($norm_text) = @_;
    $norm_text = trim($norm_text);
    my @tokens = split /\s+/, $norm_text;
    my @ntokens;
    for my $token (@tokens) {
        if ($token =~ /^https?:\/\//) {
            push @ntokens, $token;
        } else {
            push @ntokens, tokenization_orig($token);
        }
    }
    $norm_text = join ' ', @ntokens;
    return $norm_text;
}

sub tokenization_orig
{
	my ($norm_text) = @_;
...
sub printout_report
{
	if ( $METHOD eq "BOTH" )
	{
		foreach my $sys (sort @tst_sys)
		{
			printf "NIST score = %2.5f  BLEU score = %.5f for system \"$sys\"\n",$NISTmt{5}{$sys}{cum},$BLEUmt{4}{$sys}{cum};
		}
	}
	elsif ($METHOD eq "NIST" )
	{
		foreach my $sys (sort @tst_sys)
		{
			printf "NIST score = %2.5f  for system \"$sys\"\n",$NISTmt{5}{$sys}{cum};
		}
	}
